...
class RepositoryInput(unittest.TestCase):
    """The repository is an input now, so its absence must be loud.

    WHY this test exists: with REPO empty every `gh` call targets the string "", which
    lists nothing, which reads as "no open release PR" -- a green run that healed
    nothing. That is the exact silent-success shape this whole tool was built to end,
    so the guard against it gets asserted rather than assumed.
    """

    def test_an_unset_repository_is_a_failure_not_an_empty_sweep(self) -> None:
        with mock.patch.object(rpc, "REPO", ""), \
             mock.patch.object(rpc, "open_release_prs") as listing:
...
class Reporting(unittest.TestCase):
    def setUp(self) -> None:
        # WHY: every test below drives main(), which now refuses to run without a
        # repository. Without this, three of them failed and -- worse -- two PASSED for
        # the wrong reason, returning the 1 they expected from the missing-repo guard
        # rather than from the condition each was written to assert.
        patcher = mock.patch.object(rpc, "REPO", "forkwright/probe")
        patcher.start()
